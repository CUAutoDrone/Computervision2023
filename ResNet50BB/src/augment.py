...
class DataAug:
    ...
    def rotateImg(self, img, xmin, ymin, xmax, ymax, angle):
        ...
        boxEdges = np.hstack(
            (boxEdges, np.ones((boxEdges.shape[0], 1), dtype=type(boxEdges[0][0]))))
    ...
    def randomCrop(self, img, xmin, ymin, xmax, ymax):
        ...
        xstart = np.random.randint(0, xmin + (xmax-xmin)//5)
        xstop = np.random.randint(xmax - (xmax-xmin)//5, w)
        ystart = np.random.randint(0, ymin + (ymax-ymin)//5)
        ystop = np.random.randint(ymax - (ymax-ymin)//5, h)

        img = cv2.resize(img[ystart:ystop, xstart:xstop],
        ...
        xmax = w if xstop <= xmax else w - \
            (w * (xstop - xmax)/(xstop - xstart))
        ymax = h if ystop <= ymax else h - \
            (h * (ystop - ymax)/(ystop - ystart))
    ...
    def randomPerspective(self, img, xmin, ymin, xmax, ymax):
        ...
        x1, x3 = np.random.randint(
            max(0, xmin - 1. * bbWidth), xmin + bbWidth//4, size=2)
        x2, x4 = np.random.randint(
            xmax - bbWidth//4, min(w, xmax + 1. * bbWidth), size=2)
        y3, y4 = np.random.randint(
            max(0, ymin - 1. * bbHeight), ymin + bbHeight//4, size=2)
        y1, y2 = np.random.randint(
            ymax - bbHeight//4, min(h, ymax + 1. * bbHeight), size=2)

        pts1 = np.float32([[x1, y1], [x2, y2], [x3, y3], [x4, y4]])
        pts2 = np.float32([[0, h], [w, h], [0, 0], [w, 0]])

        mask = np.zeros((h, w))
        mask = cv2.rectangle(mask, (xmin, ymin),
    ...
    def motionBlur(self, img, strength, angle):
        ...
        kernel = cv2.warpAffine(kernel, cv2.getRotationMatrix2D(
            (strength / 2, strength / 2), angle, 1.0), (strength, strength))
    ...
    def modifyCSL(self, img, contrast, saturation, luminance):
        ...
        return cv2.cvtColor(np.clip(img, 0, 255).astype(np.uint8).astype("uint8"), cv2.COLOR_HSV2BGR)
    ...
    def addNoise(self, img, strength):
        ...
        h, w, c = img.shape
        noisep = strength*np.random.rand(h, w, c)
        noisem = strength*np.random.rand(h, w, c)
    ...
    def applyAug(self, imgName, funcs, params, saveImg=True):
        ...
        for func, param in zip(funcs, params):
            if func.isGeometric:
                img, xmin, ymin, xmax, ymax = func(
                    img, xmin, ymin, xmax, ymax, *param)
            else:
                img = func(img, *param)
            mods += func.name
